...
from src.utils import leer_df, guardar_df
...
def crear_gold(ruta_ttl="data/silver/silver_ttl.parquet", ruta_spi="data/silver/silver_spi.csv"):
    print("="*50, "\nCREACIÓN DE gold_metadata.parquet", "="*50)

    print("Importando archivos de capa silver...")
    ttl = leer_df(ruta_ttl, False)
    spi = leer_df(ruta_spi)

    print("Archivos importados con éxito. Merging...")
    df = merge_dataframes(ttl, spi)
    print("Limpieza básica...\n", "[1/3] Eliminación de nulos...")
    df.dropna(subset=['alto_mm', 'ancho_mm', 'peso', 'grueso', 'n_paginas', 'precio'], inplace=True)

    print("[2/3] Eliminando columnas innecesarias...")
    if 'sinopsis' in df.columns:
        df.drop(columns=['sinopsis'], inplace=True)

    print("[3/3] Eliminando outliers...\n")
    ...
    print("Creando métricas finales...")
    df = portabilidad(df)
    df = compacidad(df)
    df = prestancia(df)
    df = colaboradores(df)
    ...
    guardar_df(df, "data/gold/gold_metadata.parquet", False)
    print("gold_metadata.parquet guardado.")
